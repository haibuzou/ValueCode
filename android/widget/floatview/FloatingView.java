 ...
 * FIXME:Nexus5＋YouTube 对于应用程序，它出现在导航栏的前面
 */
class FloatingView extends FrameLayout implements ViewTreeObserver.OnPreDrawListener {
    ...
    public boolean onPreDraw() {
        ...
        // 如果为X坐标设置了初始值，则输入默认值（不考虑边距）
        if (mInitX == DEFAULT_X) {
            mInitX = 0;
        }
        // 如果为 Y 坐标设置了初始值，请输入默认值。
        if (mInitY == DEFAULT_Y) {
    ...
    private void updateNavigationBarOffset(boolean isHideNavigationBar, boolean isPortrait, Rect windowRect) {
        ...
        if (!isHideNavigationBar) {
            ...
            // 根据与其他设备的不一致进行猜测
            // 1.设备内置导航栏的高度（mBaseNavigationBarHeight == 0）不会因系统状态而异。
            // 2.设备内置的导航栏（!hasSoftNavigationBar）不一致，因为它故意将Base设置为0。
    ...
    private boolean dispatchTouchEvent(@NonNull MotionEvent event, boolean isOnInterceptTouchEvent) {
        ...
        if (action == MotionEvent.ACTION_DOWN) {
            ...
            // 通过的保持时间 确定冲压过程
            // 防止在按下后更改 mIsDraggable 或 getVisibility () 标志时处理 MOVE 等
     ...
     * 移动到左右边缘。
     *
     * @param withAnimation 如果你想要动画，则为 True。如果你不想，则为 False。
     */
     ...
     * @param startX        X坐标初始值
     * @param startY        Y坐标初始值
     * @param withAnimation 如果你想动画则为true，如果你不想动画则为false
     */
    ...
    /**
     * 移动到恒定坐标。<br/>
     * 如果超过屏幕边缘的坐标，会自动移动到屏幕边缘。
     *
     * @param currentX      当前 X 坐标（用于动画的起点）
     * @param currentY      当前 Y 坐标（用于动画的开始）
     * @param goalPositionX 目的地X坐标
     * @param goalPositionY 目的地Y坐标
     * @param withAnimation 如果你想动画则为true，如果你不想动画则为false
     */
    ...
    private void setScale(float newScale) {
        // INFO:修复了除非为child设置比例，否则放大率不会改变的现象。
     ...
     * 设置初始坐标。
     *
     * @param x FloatingView的初始X坐标
     * @param y FloatingView的初始Y坐标
     */
     ...
     * 设置标志以在初始显示时进行动画处理。
     *
     * @param animateInitialMove 初始显示动画时为true
     */
    ...
    /**
     * 根据触摸坐标计算的 FloatingView X 坐标
     *
     * @return FloatingView X 坐标
     */
    ...
    /**
     * 根据触摸坐标计算出的Floating View的Y坐标
     *
     * @return Floating View的Y坐标
     */
     ...
     * 变成重叠的状态。。
     *
     * @param centerX 对象的中心坐标X
     * @param centerY 对象的中心坐标Y
     */
    ...
    static class FloatingAnimationHandler extends Handler {
         ...
         * 更新追踪对象的位置。
         *
         * @param centerX 追随对象的X坐标
         * @param centerY 追随对象的Y坐标
         */
    ...
    /**
     * 判断长按的处理程序。<br/>
     * dispatchTouchEvent由于所有的触摸处理都是在里面实现的，长按也是独立实现的。
     */
